...
def early_stopping(stats, curr_count_to_patience, global_min_loss):
    ...
    if (new_loss:=stats[-1][1]) >= global_min_loss:
        curr_count_to_patience +=1
    else: #new loss less than global min
...
def predictions(logits):
    ...
    """
# After making a forward pass, we can use the output from the CNN to make a prediction. Implement
# the predictions() function in train common.py to predict the numeric class label given the
# model’s output. This set of outputs, called logits, are just the raw outputs of the network (without
# normalization).
# Hint: Notice that we have one logit per each class in our classification problem. How can you relate the
# index of each logit to the corresponding class label? Also consider what impacts the relative magnitude
# of each logit, and how that relates to which numeric class label we might want to predict.

    # TODO implement predictions
    pred = None # value at index i of each logit [x,y] in tensor represents likelihood of img being in the class (greater the value, the more likely)
