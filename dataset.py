...
class ImageStandardizer(object):
    ...
    def fit(self, X):
        ...
        means = np.mean(X, axis=(0,1,2)) #dimension 3 is where we the channels span, so we don't want to mean across that
        stds = np.std(X, axis=(0,1,2))
        self.image_mean = means
        self.image_std = stds
        # print(X.shape)
        print(f"len(means): {len(means)}")
        print(f"means look like: {means}")
        # print(X.shape)
        print(f"len(stds): {len(stds)}")
        print(f"stds look like: {stds}")
