...
def Rotate(deg=20):
    ...
    def _rotate(img):
        ...
        angle = np.random.randint(-1*deg, deg)
        new_img = rotate(img, angle=angle, axes=(0,1), mode='constant', cval=0, reshape=False)
        # print(img.shape)
        # print(new_img.shape)
...
def Grayscale():
    ...
    def _grayscale(img):
        ...
        avg = (np.round(np.average(img, axis = 2, keepdims=True))).astype(np.uint8) # average across C (channel axis)
        # avg = np.expand_dims(avg, axis=2)
        # print(avg)
        new_img = np.tile(avg, (1, 1, 3))
...
def main(args):
    ...
    print(f"AUGMENT_PARTITIONS: {augment_partitions}")
    # return
